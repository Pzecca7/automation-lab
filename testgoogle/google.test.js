...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("melons", Key.RETURN);
  await driver.wait(until.titleIs("melons - Google Search"), 1000);
  // TODO Call .clear() on the search bar element to clear the old search term
  await driver.findElement(By.name("q")).clear("melons")
  // TODO Call .sendKeys() on the search bar element to search for a new term
  await driver.findElement(By.name("q")).sendKeys("bananas", Key.RETURN);
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("bananas - Google Search"), 1000);
});
